...
TableauDynamique * TableauDynamique::unionTab(const TableauDynamique& tab) const
{
    TableauDynamique * res = new TableauDynamique [capacite + tab.capacite];
    res->taille_utilisee = 0;
    for(unsigned int i = 0; i < taille_utilisee; ++i) {
        res->ajouterElement(ad[i]);
    }

    for(unsigned int i = 0; i < res->taille_utilisee; ++i) {
        for(unsigned int j = 0; j < tab.taille_utilisee; ++j) {
            if(res->ad[i] == tab.ad[j])
                res->supprimerElement(i);
        }
    }

    for(unsigned int i = 0; i < tab.taille_utilisee; ++i) {
        res->ajouterElement(tab.ad[i]);
    }
